...
- **알림 시간 직접 입력** — 월별 캘린더 + 시·분 단위 직접 지정. 과거 날짜·시간은 선택 불가로 가드합니다. 반복은 `없음 / 매일 / 매주 / 매월` 중 선택하거나, 장기 환기를 위한 `수시로 + 지속 시간(10분~하루)` 모드로 지정할 수 있습니다.
- 로컬 푸시 알림 (`expo-notifications`)
- **JSON 백업/복원 — iCloud Drive 경유** — 자동 동기화 없이도 멀티 기기·기기 교체에 대응합니다.
- **Obsidian / Notion용 Markdown export** — 사용자가 이미 쓰는 지식 관리 도구로 카드를 빼낼 수 있어 앱이 데이터를 가두지 않습니다.

## 제품 결정

이 앱은 "한 사람이 자기 콘텐츠를 자기 호흡대로 다시 보게 한다"는 흐름을 핵심으로 합니다. 그 흐름을 깨지 않기 위한 결정들입니다.

| 결정 | 의도 |
|---|---|
| 일관된 친구 톤 카피 | 시스템 알림의 무덤덤함을 깨고 미루는 자신을 가볍게 자극합니다. 빈 상태부터 알림 카피, 압박 옵션까지 동일한 톤을 유지합니다. |
| **로그인 없는 온디바이스 우선** | 저장 콘텐츠는 개인 행동 데이터에 가깝다고 보고, 카드 본문은 `AsyncStorage`로 기기 내부에만 둡니다. 회원가입·SSO 흐름 자체를 제거해 첫 진입 마찰도 0으로 만들었습니다. |
| **개인 데이터와 분석 데이터의 명확한 분리** | 카드 본문(제목·링크·메모)은 서버로 절대 전송하지 않습니다. 카테고리·소요 시간 카테고리 같은 메타데이터만 익명 사용 이벤트로 Supabase에 옵션 전송합니다. |
| 분석은 옵션 | Supabase 없이도 앱이 정상 실행되도록 `lib/analytics.ts`에서 환경 변수 미설정·placeholder를 감지해 콘솔 mock 로깅으로 폴백합니다. |
| **백업은 사용자 트리거 + iCloud로 실사용성 확보** | 자동 클라우드 동기화는 신뢰·복잡도 비용이 크다고 판단했습니다. 대신 JSON export → 파일 앱/iCloud Drive 저장 → 파일 선택 import 흐름으로, 멀티 기기·기기 교체 시나리오를 사용자가 직접 제어합니다. |
| **사용자 워크플로우 존중 — Obsidian/Notion export** | 카드를 `.md`로 빼내 사용자가 이미 쓰는 지식 관리 도구에서 이어 쓸 수 있게 합니다. 앱이 데이터를 인질로 잡지 않는다는 신호이자 lock-in 회피 전략입니다. |
| **알림 UX는 정확성 우선** | 프리셋만으로는 실제 행동 시점과 어긋나기 쉽다고 보고, 분 단위 직접 입력을 기본 흐름으로 두었습니다. 과거 시간은 UI에서 차단해 "예약했는데 안 울리는" 사고를 원천 방지하고, 반복 옵션과 별개로 `수시로` 모드를 추가해 디지털 디톡스·이직 준비 같은 장기 환기에도 대응합니다. |
...
- [ ] 알림 데이터 모델을 문자열 label 중심에서 `예약 시각 + 반복 규칙` 구조로 분리하기
- [ ] `"오늘 밤 9시"`, `"내일 오전"` 프리셋도 실제 로컬 푸시 예약으로 연결하기
- [ ] `매일 / 매주 / 매월 / 수시로` 반복 옵션을 UI 표시만이 아니라 실제 스케줄링 로직까지 구현하기
- [ ] 항목을 `완료` 처리했다가 다시 되돌렸을 때 알림이 자동으로 재예약되도록 수정하기
- [ ] 백업 복원 후 현재 기기에서 알림을 다시 스케줄하도록 복원 플로우 보강하기
- [ ] 백업 JSON에서 기기 종속적인 `notificationIds`를 그대로 들고 갈지, export 시 제외할지 정책 정리하기
- [ ] README 기능 설명과 실제 구현 범위를 맞춰서 문서 과장을 없애기
