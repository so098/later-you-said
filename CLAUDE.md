...
UI 작업 시 추가 룰:
- 터치 타깃은 최소 44pt를 확보한다 (RN 기본 `TouchableOpacity` 영역이 작으면 `hitSlop` 또는 padding으로 보완).
- 신규 화면/컴포넌트를 만들 때 화려함보다 기존 톤(미니멀 모노톤)과의 정합을 우선한다 — 의도가 불명확하면 보수적으로 맞춘다.
- 디자인 작업이 기능 추가/상태 구조 변경으로 번지지 않게 한다. 로직이 필요한 변경은 기획(brainstorming)/리팩토링(refactorer) 영역.
